...
void PBListBoxItem::draw()
{
  ...
  if (_image){
    ...
    int H=h() - 3;
    if(_image->w()>W) _image->draw(x() + 3,y() + 3,W,W/_image->w()*H);
    else if(_align==ALIGN_CENTER){
      _image->draw(x() + (W-_image->w())/2,y() + 3);
    } else if(_align==ALIGN_RIGHT){
      _image->draw(x() + 3 + W-_image->w(),y() + 3);
    } else _image->draw(x() +3 ,y() + 3);
    ...
    DrawTextRect(x() + 3, y() + 3, w() - markerWidth - 3, 
    h() - 3, (char *) text.c_str(), _align);
  }else
    DrawTextRect(x() + 3 + markerWidth, y() + 3, w() - markerWidth - 3, h() - 3, (char *) getText().c_str(), _align);
  if (getMarker().size() > 0) {
...
int PBListBox::handle(int type, int par1, int par2)
{
  ...
  if( EVT_POINTERDOWN == type && eventInside(par1,par2) && (x()+2.*w()/3.)<par1 ){
    ...
    if( totalHeight > h() && scrollDelta >0  
        && par2 < (y() + 1./3.*h()) ){
      scrollDelta -= h() * 0.9;
      if (scrollDelta < 0)
        scrollDelta = 0;
...
void PBListBox::draw()
{
  ...
  if (getCaption().size() > 0) {
    DrawTextRect(x() + BORDER_SPACE, y() + BORDER_SPACE/2,
                 w() - BORDER_SPACE * 2, cH, 
                 (char *) getCaption().c_str(), ALIGN_CENTER);
    DrawLine(x() + BORDER_SPACE, 
             y() + BORDER_SPACE/2 + cH, 
             x() + w() - BORDER_SPACE, 
             y() + BORDER_SPACE + cH, BLACK);
  }
  ...
  if (totalHeight > (h() - cH) ) {
    ...
    DrawRect(x() + w() - 2*BORDER_SPACE, 
             y() + cH + 2*BORDER_SPACE, 
             BORDER_SPACE, scrH, BLACK);
    FillArea(x() + w() - 2*BORDER_SPACE, 
             y() + BORDER_SPACE / 2 + scrollDelta * scrH / (totalHeight) +cH
             , 2* BORDER_SPACE, scrH * scrH / totalHeight - BORDER_SPACE, BLACK);
  }
...
PBListBoxItem *PBListBox::erase(const PBListBoxItem* itm,bool delete_item){
  ...
  if(it!=_items.end()){
    ...
    update_needed=true;
    if(delete_item){delete erased;}
    else{ return erased;}
  }
  return NULL;
}

PBListBoxItem *PBListBox::addItem(PBImage * image, const std::string & tag,int align)
{
